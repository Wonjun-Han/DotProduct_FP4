...
class p_Expansion_Groupwise_CSA_BLOCK_IO(depthBitWidth: Int, accWidth: Int, vecSize: Int, outWidth: Int) extends Bundle {
  val in        = Input(Vec(vecSize, SInt(accWidth.W)))
  val exponent  = Input(Vec(vecSize, SInt(9.W)))
  ...
  val out_exponent_gmax   = Output(Vec(vecSize / 2, SInt(9.W)))
}

class p_Expansion_Groupwise_CSA(val d: Int, val extra: Int) extends Module {
  val accWidth = 9 + d
  val outWidth = 8 + d + extra
  val vecSize = 256 >> d // d = 5 → vecSize = 8

  val io = IO(new p_Expansion_Groupwise_CSA_BLOCK_IO(
    depthBitWidth = 4,
    accWidth = accWidth,
    vecSize = vecSize,
    outWidth = outWidth
  ...
  // ------------------------------
    def minS9: SInt = (-256).S(9.W)  // 무효 후보 대체값(최솟값)
    ...
    val grouped_max_exp = Wire(Vec(vecSize/2, SInt(9.W)))
    ...
    // 그룹 내 후보(최대 8개)만 만들고, 무효는 minS9으로 마스킹, 그리고 만약에 accum=0이면 minS9으로 대체하자
    val cands: Seq[SInt] = (0 until 8).map { k =>
      ...
      val expEff = Mux(laneZero(idx), minS9, io.exponent(idx))
      Mux(ok, expEff, minS9)
    }
    ...
    grouped_max_exp(g) := Mux(active, grpMax, 0.S(9.W))
  ...
  // ------------------------------
    ...
    io.out_exponent_gmax(i) := Mux(enable && (i.U < expansion_num), grouped_max_exp(i), 0.S(9.W))
    }
