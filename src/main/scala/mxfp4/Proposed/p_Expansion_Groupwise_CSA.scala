...
class p_Expansion_Groupwise_CSA_IO(depthBitWidth: Int, accWidth: Int, vecSize: Int, outWidth: Int) extends Bundle {
...
class p_Expansion_Groupwise_CSA(val d: Int, val extra: Int) extends Module {
  ...
  val io = IO(new p_Expansion_Groupwise_CSA_IO(
    depthBitWidth = 4,
    accWidth      = accWidth,
    vecSize       = vecSize,
    outWidth      = outWidth
  ...
  for (g <- 0 until vecSize/2) {
    ...
    // 그룹 내 후보(최대 8개)만 만들고, 무효는 minS9으로 마스킹
