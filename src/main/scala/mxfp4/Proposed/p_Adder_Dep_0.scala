...
class MXFP4_ADD_DEPTH0_BLOCK_IO extends Bundle {
  val sign     = Input(Vec(32, UInt(1.W)))
  val mantissa = Input(Vec(32, UInt(8.W)))   // 4-bit mantissa + padding
  val depth    = Input(UInt(3.W))            // Optional
  val out = Output(Vec(16, SInt(9.W)))       // SInt로 직접 표현
}

class p_Adder_Dep_0 extends Module {
  val io = IO(new MXFP4_ADD_DEPTH0_BLOCK_IO)
