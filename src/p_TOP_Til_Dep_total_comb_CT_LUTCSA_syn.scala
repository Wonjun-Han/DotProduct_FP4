...
class p_TOP_Til_Dep_total_COMB_CT_LUTCSA_SYN_BLOCK_IO extends Bundle {
...
class p_TOP_Til_Dep_total_comb_CT_LUTCSA_syn extends Module {
  val io = IO(new p_TOP_Til_Dep_total_COMB_CT_LUTCSA_SYN_BLOCK_IO)
  ...
  val LUT_2D = Module(new p_LUT_2D)
  val CSA_GROUP = Seq.fill(8)(Module(new p_CSA_Total(9, 32, 14)))
  val adder_sumcarry = Module(new p_Adder_sumcarry(14))
  ...
  val CSA_GROUPWISE = Module(new p_CSA_Total(44, 8, 47)) // 44 = 14 + 30(extra)
...
*/
    ...
    CSA_GROUP(i).io.sign     := expansion.io.out_sign
    CSA_GROUP(i).io.mantissa := expansion.io.out_mantissa
  }
...
*/
  // ---------------------------------
  for (i <- 0 until 2) {
      adder_sumcarry.io.in(i) := CSA_GROUP.io.out(i)
  }
