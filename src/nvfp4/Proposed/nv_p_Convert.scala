...
class NVFP4_CONVERT_BLOCK_IO(depthBitWidth: Int, accWidth: Int, vecSize: Int, absWidth: Int) extends Bundle {
...
class nv_p_Convert(val d: Int) extends Module {
  val accWidth = 9 + d
  val vecSize = 256 >> d
  val absWidth = 8 + d

  val io = IO(new NVFP4_CONVERT_BLOCK_IO(depthBitWidth = 4, accWidth = accWidth, vecSize = vecSize, absWidth = absWidth))
