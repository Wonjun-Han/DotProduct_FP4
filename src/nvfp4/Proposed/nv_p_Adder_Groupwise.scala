...
class NVFP4_ADD_GROUPWISE_BLOCK_IO (depthBitWidth: Int, mantissaWidth: Int, vecSize: Int, outWidth: Int) extends Bundle {
...
class nv_p_Adder_Groupwise(val d: Int, val extra: Int) extends Module {
    val mantissaWidth = 7 + d + extra 
    val outWidth = mantissaWidth + 1 // 1 bit for overflow
    val vecSize = 256 >> (d-1)

    val io = IO(new NVFP4_ADD_GROUPWISE_BLOCK_IO(
        depthBitWidth = 4,
