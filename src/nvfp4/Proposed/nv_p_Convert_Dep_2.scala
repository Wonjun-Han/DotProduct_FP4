...
class NVFP4_CONVERT_DEPTH2_BLOCK_IO extends Bundle {
  ...
  val out      = Output(Vec(64, new FP32))      // FP32 결과

  // for debugging
  val debug_real_exp   = Output(Vec(64, SInt(10.W)))
  val debug_biased_exp = Output(Vec(64, SInt(10.W)))
  val debug_shift_amt  = Output(Vec(64, SInt(5.W)))
  val debug_PE         = Output(Vec(64, UInt(4.W)))
  val debug_abs_in     = Output(Vec(64, UInt(10.W)))
}

class nv_p_Convert_Dep_2 extends Module {
  val io = IO(new NVFP4_CONVERT_DEPTH2_BLOCK_IO)
  ...
  for (i <- 0 until 64) {
    ...
    val abs_val_full = Mux(input < 0.S, -input, input).asUInt // UInt(10.W)

    // ✅ 절댓값의 MSB는 항상 0 (Q8.6 최대값 8191)
    val abs_val = abs_val_full(9, 0) // MSB 제거 → UInt(10.W)

    // ---------------------
    // Step 2: PriorityEncoder + shift
    // ---------------------
    val PE = PriorityEncoder(Reverse(abs_val))    // 최상위 1bit 위치
    val shift_amt = (3.S - PE.asSInt).asSInt      // 고정소수점 기준 정규화 shift (소수점 기준)
    ...
    // Step 3: Exponent 계산
    // ---------------------
    val real_exp   = io.exponent(i/8) + shift_amt    // scale_sum + emax + 정규화 shift
