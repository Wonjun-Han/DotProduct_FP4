...
class NVFP4_CONVERT_DEPTH1_BLOCK_IO extends Bundle {
  val depth    = Input(UInt(4.W))
  val in       = Input(Vec(128, SInt(10.W)))     // 고정소수점 누적합
  val nan      = Input(Vec(8, UInt(1.W)))      // NaN 여부 flag
  val exponent = Input(Vec(8, SInt(10.W)))     // ScaleEmax 결과
  val out      = Output(Vec(128, new FP32))      // FP32 결과

  // for debugging
  val debug_real_exp   = Output(Vec(128, SInt(10.W)))
  val debug_biased_exp = Output(Vec(128, SInt(10.W)))
  val debug_shift_amt  = Output(Vec(128, SInt(5.W)))
  val debug_PE         = Output(Vec(128, UInt(4.W)))
  val debug_abs_in     = Output(Vec(128, UInt(9.W)))
}


// SInt.sign -> 이런 거 쓰는게 좋을 수 있음.(어떤 식으로 ...)
class nv_p_Convert_Dep_1 extends Module {
  val io = IO(new NVFP4_CONVERT_DEPTH1_BLOCK_IO)
  ...
  for (i <- 0 until 128) {
    ...
    val shift_amt = (2.S - PE.asSInt).asSInt      // 고정소수점 기준 정규화 shift (소수점 기준)
    ...
    // Step 3: Exponent 계산
    // ---------------------
    val real_exp   = io.exponent(i/16) + shift_amt    // scale_sum + emax + 정규화 shift
