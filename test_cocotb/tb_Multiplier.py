...
async def test_multiplier_random(dut):
    ...
    for i in range(256):
        ...
        expected_bias = calc_bias(a_exp, b_exp)
        ...
        bias_val     = getattr(dut, f"io_bias_{i}").value.integer
        ...
        assert bias_val == expected_bias,     f"bias[{i}] mismatch: got {bias_val}, expected {expected_bias}"
