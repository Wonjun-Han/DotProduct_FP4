...
async def test_mxfp4_mac_all_depths_new(dut):
    for depth in range(0,6):
        ...
        group_size = 32 >> depth
        products_per_output = 256 // output_count
        ...
        dut._log.info("────────────────────────────────────────────────────────────")
        dut._log.info("✅ All testbenches successfully verified across depth levels 0–5")
        dut._log.info(f"🔁 Trials per depth         : {NUM_TRIALS}")
        dut._log.info(f"📊 Total validation runs    : {NUM_TRIALS * 5}")
        dut._log.info("🏗️  DUT Overview            : p_TOP_Til_Dep_ALL_V1")
        ...
        dut._log.info("📎 Ready for integration.")
        dut._log.info("────────────────────────────────────────────────────────────\n")
