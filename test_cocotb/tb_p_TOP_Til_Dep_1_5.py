...
@cocotb.test()
async def test_p_top_til_dep_1_5(dut):
    ...
    for depth in range(1, 6):
        ...
        # Output count = 256 >> depth
        output_count = 256 >> depth

        for i in range(output_count):
            group_idx = i // (32 >> depth)
            group_base = group_idx * (32 >> depth)
            group_sum = 0.0

            for j in range(32 >> depth):
                idx = group_base + j
    ...
    for trial in range(NUM_TRIALS):
        ...
        for group_idx in range(8):
            ...
            if (scaled_sum != 0.0) and (
                ...
                raise AssertionError(
                    f"\n[❌ Trial {trial + 1} | DEPTH 5 - Group {group_idx}]\n"
                    f"🔸 Group Sum (pre-scale): {group_sum:.6e}\n"
                    f"🔸 Scale Sum: {scale_exp} → Scaled Sum: {scaled_sum:.6e}\n\n"
                    f"🧪 Expected FP32:\n"
                    f"  - Sign     : {expected_sign} (0b{expected_sign:01b})\n"
    ...
    dut._log.info(f"✅ All {NUM_TRIALS} trials passed for Depth 5!")
