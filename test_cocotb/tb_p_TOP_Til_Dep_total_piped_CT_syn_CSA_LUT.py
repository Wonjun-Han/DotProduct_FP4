...
# Config
# =========================================================
NUM_TRIALS = 2000            # per-depth, 스트리밍 검증 시 transaciton 수
PIPELINE_LATENCY = 10       # 설계 고정 레이턴시 (입력 cycle t -> 출력 cycle t+11)
...
ALLOWED_DEPTHS = (0, 5, 8)  # ← 추가: 혼합 스트림에서 허용할 depth 집합
...
async def stream_and_check_mixed_depths(dut, num_transactions: int, pattern: str = "roundrobin"):
    ...
    큐[t-11]의 '그 트랜잭션 depth'로 계산한 기대값과 14 사이클 이후에 비교.
    """
    
    # depth 시퀀스
    if pattern == "roundrobin":
        # 0, 5, 8, 0, 5, 8, ...
        depth_seq = [ALLOWED_DEPTHS[i % len(ALLOWED_DEPTHS)] for i in range(num_transactions)]
    elif pattern == "random":
        # {0,5,8} 중 랜덤
        depth_seq = [random.choice(ALLOWED_DEPTHS) for _ in range(num_transactions)]
    else:
...
async def test_mxfp4_mac_pipelined_mixed_depths_streaming(dut):
    ...
    dut._log.info("\n\n📘 [MXFP4 LUTCSA Pipelined Dot-Product Verification Summary]")
    dut._log.info("────────────────────────────────────────────────────────────")
    dut._log.info("✅ All pipelined testbenches successfully verified across depth levels 0/5/8")
    ...
    dut._log.info("   └─ 10-stage pipelined MXFP4 MAC with scale-aware accumulation")
    dut._log.info("   └─ Supports dynamic depth control for accumulation tree (0/5/8)")
