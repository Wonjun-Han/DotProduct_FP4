...
# =========================================================
# Config (COMB: front/back reg only → total latency = 2)
# =========================================================
NUM_TRIALS = 10000            # per-depth, 스트리밍 검증 시 transaciton 수
PIPELINE_LATENCY = 3          # 입력 t -> 출력 t+3 (앞단 입력 래치 + 조합 + 뒷단 출력 FF)
...
async def stream_and_check(dut, depth: int, num_transactions: int):
    """
    매 사이클 입력 연속 주입.
    출력은 항상 2사이클 뒤이므로 큐[t-2] 기대값과 현재 출력을 비교.
    """
...
async def stream_and_check_mixed_depths(dut, num_transactions: int, pattern: str = "roundrobin"):
    ...
    출력은 항상 입력으로부터 2사이클 뒤 결과이므로,
    큐[t-2]의 '그 트랜잭션 depth' 기대값과 비교.
    """
    # depth 시퀀스
...
async def test_mxfp4_mac_pipelined_single_depth_streaming(dut):
    """단일 깊이에서 백투백 스트리밍 입력을 넣고 t-2 매칭으로 검증."""
...
async def test_mxfp4_mac_pipelined_mixed_depths_streaming(dut):
    """혼합 depth(0~8)를 라운드로빈/랜덤으로 매 사이클 변경하며 스트리밍 검증 (t-2 매칭)."""
    cocotb.start_soon(Clock(dut.clock, 10, units='ns').start())
    await reset_dut(dut)
    ...
    dut._log.info("\n\n📘 [MXFP4 COMB Dot-Product Verification Summary]")
    dut._log.info("────────────────────────────────────────────────────────────")
    dut._log.info("✅ All comb-mode testbenches successfully verified across depth levels 0–8")
    ...
    dut._log.info("   └─ Includes pipeline timing alignment and throughput validation")
    dut._log.info("   └─ Handles NaN propagation across depth paths")
    dut._log.info("────────────────────────────────────────────────────────────")
    dut._log.info("🎯 Result  : ✅ All comb-mode functional correctness tests passed")
    dut._log.info("📦 Module  : p_TOP_Til_Dep_total_COMB")
    dut._log.info("────────────────────────────────────────────────────────────\n")
